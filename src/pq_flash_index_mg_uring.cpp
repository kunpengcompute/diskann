...
int PQFlashIndexMGV2<T, LabelT>::load(uint32_t num_threads, const char *index_prefix, const char *mem_graph_path)
{
    ...
    else
    {
        ...
        int graph_ret = load_graph_to_memory(graph_path);
        if (graph_ret != 0)
        {
            diskann::cout << "Error: Failed to load memory graph from " << graph_path << std::endl;
            return -1;
        }
    }
                                                        ...
                                                        QueryStats *stats)
{
    ...
    uint32_t reorder_vector_num = static_cast<uint32_t>(l_search);
