...
DiskANN是由微软团队提出的面向大规模向量近似最近邻搜索（ANN）的基于磁盘的图索引算法，能在有限内存下支持单机亿级向量检索，通过将PQ压缩向量驻留内存、完整索引与原始向量持久化于SSD实现IO高效的检索。原始实现基于x86_64 AVX2指令集。鲲鹏优化基于开源DiskANN v0.7.0代码做侵入式修改，将其扩展至ARM64（AArch64）架构，引入NEON SIMD向量化、数据布局优化（邻接表驻留内存）、异步IO流水化、精排队列缩减等多项性能优化。
